...
export default async function handler(req, res) {
  ...
  try {
    ...
    const filename = req.method === "POST" ? body.filename : query.filename;
    const type = req.method === "POST" ? body.type : query.type;

    if (!filename || !type) {
      console.log("Missing filename or type", { filename, type, body, query });
